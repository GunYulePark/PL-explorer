...
export function PnlWorkspace() {
  ...
  useEffect(() => {
    ...
    async function loadRows(activeClient: NonNullable<typeof client>) {
      setLoading(true);
      let query = activeClient.from("pl_facts").select("account_code,amount").eq("fiscal_year", Number(filters.year)).in("account_code", statementOrder.map((row) => row.code));
  ...
  }, [configuredClient, filters, isAuthenticated]);
  ...
  function downloadCsv() {
    const data = [["손익 항목", `${filters.year} ${filters.period}`, "행 구성", "열 구성"], ...rows.map((row) => [row.label, row.amount, layout.rows.join(" > "), layout.columns.join(" > ")])];
  ...
  return <main className="analysis-app">
    ...
    <div className="analysis-shell">
      ...
      <section className="builder-area">
        ...
        <section className="axis-workbench"><div className="axis-card"><div className="axis-title"><span>행</span><small>끌어서 순서·영역 변경</small></div><div className={dragOverAxis === "rows" ? "chip-zone drop-active" : "chip-zone"} onDragOver={(event) => allowAxisDrop(event, "rows")} onDrop={(event) => finishAxisDrop(event, "rows")} onDragLeave={() => setDragOverAxis(null)}>{layout.rows.length ? layout.rows.map((item) => <button className={draggedItem?.item === item ? "axis-chip row dragging" : "axis-chip row"} key={item} draggable onDragStart={(event) => startAxisDrag(event, "rows", item)} onDragEnd={endAxisDrag} onDragOver={(event) => allowAxisDrop(event, "rows")} onDrop={(event) => finishAxisDrop(event, "rows", item)} onClick={() => removeFromAxis("rows", item)} title="끌어서 행·열을 바꾸거나 순서를 조정하세요">{item}<b>×</b></button>) : <span className="empty-zone">여기에 항목을 놓으세요</span>}</div></div><div className="axis-card"><div className="axis-title"><span>열</span><small>끌어서 순서·영역 변경</small></div><div className={dragOverAxis === "columns" ? "chip-zone drop-active" : "chip-zone"} onDragOver={(event) => allowAxisDrop(event, "columns")} onDrop={(event) => finishAxisDrop(event, "columns")} onDragLeave={() => setDragOverAxis(null)}>{layout.columns.length ? layout.columns.map((item) => <button className={draggedItem?.item === item ? "axis-chip column dragging" : "axis-chip column"} key={item} draggable onDragStart={(event) => startAxisDrag(event, "columns", item)} onDragEnd={endAxisDrag} onDragOver={(event) => allowAxisDrop(event, "columns")} onDrop={(event) => finishAxisDrop(event, "columns", item)} onClick={() => removeFromAxis("columns", item)} title="끌어서 행·열을 바꾸거나 순서를 조정하세요">{item}<b>×</b></button>) : <span className="empty-zone">여기에 항목을 놓으세요</span>}</div></div></section>
        ...
        <section className="search-panel"><div><strong>조회 설계</strong><span>행과 열을 정하고 필요한 필터를 적용한 뒤 분석합니다.</span></div><button className="detail-search">상세검색</button></section>
        <section className="report-panel"><div className="report-heading"><div><p>미리보기</p><h2>{activePreset?.name ?? "사용자 지정 분석"}</h2></div><div className="report-meta"><span>{layout.rows.join(" · ") || "행 없음"}</span><b>×</b><span>{layout.columns.join(" · ") || "열 없음"}</span></div></div><div className="report-table"><div className="report-row report-header"><span>구분</span><span>{filters.year} {filters.period}</span></div>{rows.map((row) => <div className={row.code === "operating_profit" ? "report-row strong" : "report-row"} key={row.code}><span>{row.label}</span><b>{loading ? "불러오는 중…" : amount(row.amount)}</b></div>)}</div><footer>{source === "sample" ? "Supabase 연결 후 저장된 손익 데이터를 조회할 수 있습니다." : "Supabase 적재 데이터를 기준으로 표시 중입니다."}<span>영업이익률 {operatingMargin ?? "-"}%</span></footer></section>
